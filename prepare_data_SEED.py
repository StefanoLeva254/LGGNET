...
class PrepareDataSeed:
    ...
    def run(self, subject_list, split=False, expand=True):
        ...
        for sub in subject_list:
            ...
            label_ = [1,2,3,0,2,0,0,1,0,1,2,1,1,1,2,3,2,2,3,3,0,3,0,3]

            if expand:
                # expand one dimension for deep learning(CNNs)
                print(type(data_))
                data_ = self.convertDictToArray(data_)
                data_ = np.expand_dims(data_, axis=-3)
                print('data:' + str(data_.shape) + ' label:' + str(len(label_)))
                print('Data dimension expanded!')
    ...
    def load_data_per_subject(self, sub):
        ...
        sub_code = str(str(sub) + '.mat')

        sub_name = self.select_name(sub=sub)

        subject_path = os.path.join(self.data_path, sub_code)
        subject = sp.loadmat(subject_path)
        data = subject
        ...
        print('data:' + '[' + str(len(data[sub_name + '_eeg1'])) + ',' + str(len(data[sub_name + '_eeg1'][0])) + ']')
        return data
    ...
    def convertDictToArray(self, data):
        """
        This function is required to convert the dictionary-shaped data into 
        a 3D-Array
        """
   
        empty_list = []
        for k in sorted(data.keys()):
            empty_list.append(data[k])
        data_ = np.array(empty_list, dtype=object)
        print(type(data_))
        print(data_.shape)
        print(type(data_[0][0]))
        print(len(data_[0][0]))
        for i1 in range(len(data_)):
            for i2 in range(len(data_[i1])):
                data_[i1][i2] = data_[i1][i2][0:7680]
        print(data_.shape)
        shape = data_.shape + (len(data_[0][0]),)
        data__ = np.empty(shape=shape)
        for i1 in range(len(data_)):
            for i2 in range(len(data_[i1])):
                for i3 in range(len(data_[i1][i2])):
                    data__[i1][i2][i3] = data_[i1][i2][i3]
        return data__
