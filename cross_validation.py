...
class CrossValidation:
    ...
    def load_per_subject(self, sub):
        ...
        seconds = data.shape[4] / rate
        
        #x = np.linspace(0,seconds,data.shape[4])
        #plt.plot(x,standard_data[0,0,0,0,:])
        #plt.show()
        #print('printing data')
        #print(data)
    ...
    def prepare_data(self, idx_train, idx_test, data, label):
        ...
        if self.args.dataset == 'SEED' or self.args.dataset == 'DEAP':
            """
    ...
    def n_fold_CV(self, subject=[0], fold=10, shuffle=True):  #fold = 10 
        """
        ...
        for sub in subject:
            ...
            fold_results = np.zeros((10,self.args.max_epoch))

            for idx_fold, (idx_train, idx_test) in enumerate(kf.split(data)):
                print('Outer loop: {}-fold-CV Fold:{} SUB:{}'.format(fold, idx_fold,sub))
                ...
                if self.args.reproduce:
                    ...
                    # to train new models
                    fold_results[idx_fold], acc_val, f1_val = self.first_stage(data=data_train, label=label_train,
                                                       subject=sub, fold=idx_fold)
            ...
            fold_means = np.zeros((self.args.max_epoch))
            for i in range(self.args.max_epoch):
                fold_means[i] = np.mean(fold_results[:,i])
            
            #plot results per epoch
            x = np.linspace(0,self.args.max_epoch,num=self.args.max_epoch)
            plt.plot(x, fold_means, label="{}".format(sub+1))
            plt.xlabel('Epoche')
            plt.ylabel('Accuratezza')
            plt.subplots_adjust(right = 0.85)
            plt.legend(bbox_to_anchor=(1.18, 1))
            plt.title('Grafico dell\' accuratezza all\'aumentare delle epoche')
 
            plt.savefig("output.jpg")
            plt.show()
            tva.append(va_val.item())
            tvf.append(vf_val.item())
            acc, f1, cm = get_metrics(y_pred=preds, y_true=acts, classes=[0,1,2,3])
            print('Subject {} Confusion Matrix:'.format(sub+1))
            print(cm)

            """x = np.linspace(0,fold)
            plt.plot(x,accuracy)
            plt.show()
            """
            tta.append(acc)
    ...
    def first_stage(self, data, label, subject, fold):
        ...
        va_item = []
        maxAcc = 0.0
        results_per_epoch = np.zeros((3,self.args.max_epoch))
        for i, (idx_train, idx_val) in enumerate(kf.split(data)):
            ...
            results_per_epoch[i], acc_val, F1_val = train(args=self.args,
        ...
        mean_fold = np.zeros(self.args.max_epoch)
        print(results_per_epoch)
        for idx in range(self.args.max_epoch) :
            mean_fold[idx] = np.mean(results_per_epoch[:,idx])
