...
def train(args, data_train, label_train, data_val, label_val, subject, fold):
    ...
    fpath = Path(args.load_path_final)
    if(fpath.is_file()):
    ...
    counter = 0
    result_per_epoch = np.zeros((args.max_epoch))
    for epoch in range(1, args.max_epoch + 1):
        ...
        result_per_epoch[epoch - 1] = acc_val
    ...
    return result_per_epoch, trlog['max_acc'], trlog['F1']
...
def test(args, data, label, reproduce, subject, fold):
    ...
    if reproduce:
        ...
        model.load_state_dict(torch.load(load_path_final,map_location=torch.device('cpu')))
    else:
        model.load_state_dict(torch.load(args.load_path_final))
    loss, pred, act = predict(
        data_loader=test_loader, net=model, loss_fn=loss_fn
    )
    acc, f1, cm = get_metrics(y_pred=pred, y_true=act, classes=[0,1,2,3])
    print(cm)
